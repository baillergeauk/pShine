...
pcocc_templates_dir="${PCOCC_USER_CONF_DIR:-"$HOME"/.pcocc}"/templates.d

create_target() {

    if [[ -d "${diskfolder:?}"/"$fsname" ]]; then
        read -p "$fsname already exists, do you want to overwrite it ? (y/n)" -r
        if [[ $REPLY =~ ^[Yy]$ ]]; then
...
write_pcocc_template() {

    ## Base template

    cat > "$pcocc_templates_dir"/"$fsname".yaml <<EOF
$fsname/$server_image:
    image: '$server_image'
    resource-set : "default"
    description : "pShine template for /$fsname"
    user-data: $pcocc_templates_dir/$fsname.cloud-config
EOF
...
EOF
        ...
        for j in $(seq 0 $((mdt_count - 1)));do
            ...
            t+=1
        done >> "$pcocc_templates_dir"/"$fsname".yaml

        if [[ "$i" -eq 0 ]]; then
            echo "       - '$diskfolder/$fsname/mgt'" >> "$pcocc_templates_dir"/"$fsname".yaml
        fi
        args+="$fsname/mds$i,"
    done
...
EOF
    ...
    echo "#client_count:$client_count" >> "$pcocc_templates_dir"/"$fsname".yaml

    # saving diskfolder to delete disks with pShineDelete

    echo "#diskfolder:$diskfolder" >> "$pcocc_templates_dir"/"$fsname".yaml
...
users:
    - name: root
      ssh-authorized-keys:
        -" > "$pcocc_templates_dir"/"$fsname".cloud-config

    cat "$HOME"/.ssh/id_rsa.pub >> "$pcocc_templates_dir"/"$fsname".cloud-config

    # preserve_hostname is needed so all the VMs do not keep the same hostname
    # else, shine can not work properly

    cat >> "$pcocc_templates_dir"/"$fsname".cloud-config <<EOF
...
EOF
    ...
    for i in $(seq 0 $((mds_count-1))); do
        ...
        for j in $(seq 0 $((mdt_count-1))); do
        echo "        mdt: node=vm$i dev=/dev/vd${tab[j]}" >> "$pcocc_templates_dir"/"$fsname".cloud-config
        t=$((t + 1))
        done
        if [[ "$i" -eq 0 ]]; then
        echo "        mgt: node=vm$i dev=/dev/vd${tab[t]}" >> "$pcocc_templates_dir"/"$fsname".cloud-config
        fi
    done

    for i in $(seq "$mds_count" $((oss_count+mds_count-1))); do
        for j in $(seq 0 $((ost_count-1))); do
        echo "        ost: node=vm$i dev=/dev/vd${tab[j]}" >> "$pcocc_templates_dir"/"$fsname".cloud-config
        done
    done

    ## Optional parameters ##

    echo "$options" | sed 's/^/        /g' >> "$pcocc_templates_dir"/"$fsname".cloud-config
}

create () {

    local total_mdt total_ost total_vm args tab
    local -i t
    if [[ ! -f "$1" ]]; then
    ...
    mkdir -p "$pcocc_templates_dir";

    ### Creating targets ###
    create_target

    ### Writing pcocc templates ###
    write_pcocc_template
...
delete () {
    ...
    if ! [[ -e "$pcocc_templates_dir/$1.yaml" ]]; then
    ...
    echo "$pcocc_templates_dir"/"$1".* | tr ' ' '\n'

    read -p "Are you sure you want to delete all the listed files ?(y/n)" -r
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        rm -rf "${diskfolder:?}"/"$1"
        rm -f "$pcocc_templates_dir"/"$1".*
    fi
}

start () {

    local pcocc_args="" fsname="" clients args
    ...
    if [[ "$fsname" = "" ]]; then
         echo "Needed option: -f [filesystem]"
         exit 64
    fi
...
EOF
    ...
    pcocc alloc $pcocc_args-E "$file" "$args""$fsname"/client:$((client-1))
...
usage() {
    ...
    Start a previously created one with desired pcocc options:
        ./pShine start [PCOCC_ALLOC_OPTIONS] -f FSNAME
